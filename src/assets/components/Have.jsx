...
// Array of text labels corresponding to images
...
const Have = () => {
  return (
    <section className="about-us flex flex-col items-center py-10 pt-28">
      <h2 className="text-5xl md:text-3xl font-bold text-cl text-center pb-16">Have a Project?</h2>
      <div className="p-6 md:p-10 rounded-lg w-full flex flex-col items-center justify-center min-h-[70vh]">  
        {/* Grid layout for images */}
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-10 md:gap-12 -mt-16">
          {[Img1, Img2, Img3, Img4].map((imgSrc, index) => (
            <div key={index} className="relative w-full h-auto lg:w-60 lg:h-96 md:w-44 md:h-48 sm:w-28 sm:h-40">
              {/* Image with text overlay */}
              <img 
                src={imgSrc} 
                alt={`Process ${index + 1}`} 
                className="w-full h-full object-cover rounded-xl md:h-60" 
                style={{ height: '32rem' }} 
              />
        ...
        <a
          ...
          className="bg-cl/90 hover:bg-cl/100 text-white font-bold py-4 px-6 md:px-12 rounded-xl items-center justify-center text-center lg:translate-y-40 mt-1 mb-16 md:mt-96"
